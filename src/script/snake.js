...
var _ticker = require('../lib/utils/ticker');

var _ticker2 = _interopRequireDefault(_ticker);

var _timer = require('../lib/utils/timer');

var _timer2 = _interopRequireDefault(_timer);

var _randomList = require('../lib/utils/randomList');
...
var control = function () {
		...
		Reflect.defineProperty(this, "speed", {
			...
			set: function set(value) {
				if (this.speedScalar !== value) {
					...
					// 更新 timer
					_timer2.default.set(this.intervalID, { delay: this.interval });
				}
			}
		});
	...
	_createClass(control, [{
		...
		value: function init() {
			...
			// 添加 ticker
			_ticker2.default.addEventListener("tick", this.tickHandle);
			// 默认暂停 ticker
			this.pause();

			var _config$width = config.width,
			    width = _config$width === undefined ? 640 : _config$width,
			    _config$height = config.height,
			    height = _config$height === undefined ? 640 : _config$height,
			    _config$row = config.row,
			    row = _config$row === undefined ? 50 : _config$row,
			    _config$column = config.column,
			    column = _config$column === undefined ? 50 : _config$column,
			    _config$border = config.border,
			    border = _config$border === undefined ? 0x999999 : _config$border,
			    _config$color = config.color,
			    color = _config$color === undefined ? 0x000000 : _config$color,
			    _config$food = config.food,
			    food = _config$food === undefined ? color : _config$food,
			    _config$min = config.min,
			    min = _config$min === undefined ? 3 : _config$min,
			    _config$speed = config.speed,
			    speed = _config$speed === undefined ? 1 : _config$speed;
			...
			// 初始化 model
			this.model.init({ row: row, column: column, min: min });
		...
		value: function destroy() {
			// 移除 ticker
			_ticker2.default.removeEventListener("tick", this.tickHandle);
			// 清空 timer
		...
		key: 'isAdverse',
		value: function isAdverse(directionA, directionB) {
			var indexA = this.fourDirections.indexOf(directionA),
		...
		value: function update() {
			...
			var data = { snake: this.model.snake, food: this.model.food };
			if (this.model.dirty) {
				// model 有变化
				var hasEatEvent = false;
		...
		key: 'tickHandle',
		value: function tickHandle() {
			_timer2.default.update(_ticker2.default.paused, _ticker2.default.elapsedMS * 1000);
			if (!_ticker2.default.paused) {
...
var _chain = require('../lib/utils/chain');
...
var model = function () {
	...
	function model() {
		...
		this.snake.shift = function () {
			var index = shift.call(_this.snake).data;
			// 更新 zone
			_this.updateZone(index, undefined, "shift");
		};

		this.snake.push = function (index) {
			push.call(_this.snake, index);
			// 更新 zone
			_this.updateZone(index, "snake", "push");
		};

		this.snake.pop = function () {
			var index = pop.call(_this.snake).data;
			// 更新 zone
			_this.updateZone(index, undefined, "pop");
		};

		// 投食后自动更新 zone
		Reflect.defineProperty(this, "food", {
			get: function get() {
				return Reflect.get(_this, "_food");
			},
			set: function set(value) {
				// 将值记录到 _food
	...
	_createClass(model, [{
		...
		value: function neighbour() {
			...
			return (0, _randomList2.default)([this.leader.left, this.leader.right, this.leader.up, this.leader.down], 1, function (index) {
				return index !== -1 && _this2.zone[index].fill === undefined;
			});
		}
		...
		value: function move(direction) {
			var index = this.leader[direction],
			    skipTail = false;
			if (-1 === index) {
				// 撞墙
...
require('../lib/gsap/TweenMax');

require('../lib/utils/getContentBoxSize');

require('../lib/utils/noHello');
...
var view = function () {
	...
	function view(dom) {
		...
		// 创建一个 app
		var app = new PIXI.Application(750, 1206, {
			transparent: true
		});
		...
		var _snake = this.snake,
		    ...
		    pop = _snake.pop,
		    push = _snake.push,
		    insertAfter = _snake.insertAfter;

		// 封闭 shift

		this.snake.shift = function () {
			// 回收尾节点
			_this.collect(shift.call(_this.snake).node);
		};

		// 封装 unshift 
		this.snake.unshift = function (data) {
			unshift.call(_this.snake, data);
			var node = _this.snake.first().node = _this.calloc();
			node.setPostion.apply(node, _toConsumableArray(_this.getPosition(data)));
		};
		...
		// 封装 push 
		this.snake.push = function (data) {
			push.call(_this.snake, data);
			var node = _this.snake.last().node = _this.calloc();
			node.setPostion.apply(node, _toConsumableArray(_this.getPosition(data)));
		};

		// 封装 insertAfter 
		this.snake.insertAfter = function (index, data) {
			insertAfter.call(_this.snake, index, data);
			var node = _this.snake.at(index + 1).node = _this.calloc();
			node.setPostion.apply(node, _toConsumableArray(_this.getPosition(data)));
		};
		...
		// 扩展 PIXI.Graphics
		PIXI.Graphics.prototype.setPostion = function (x) {
	...
	_createClass(view, [{
		...
		value: function init() {
			...
			try {
				for (var _iterator = this.data.snake[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
		...
		value: function destroy() {
			...
			try {
				for (var _iterator2 = stage.children[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
		...
		value: function drawBounds() {
			...
			var bounds = new PIXI.Graphics().beginFill(0xffffff, 1).lineStyle(thickness, border, 1).drawRect(0, 0, width + thickness, height + thickness);
			...
			// 活动空间定位
			var _ref = [bounds.x + thickness / 2, bounds.y + thickness / 2];
			this.zone.x = _ref[0];
			this.zone.y = _ref[1];
		}
		...
		key: 'createQuickMap',
		value: function createQuickMap(map) {
		...
		value: function feed(index) {
			...
			(_food = this.food).setPostion.apply(_food, _toConsumableArray(this.getPosition(index)));
		}
		...
		key: 'updateDelta',
		value: function updateDelta(snakeA) {
			var _this2 = this;

			var snakeB = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : this.snake;

			// snakeA === model.snake, snakeB === view.snake
			this.updateTail(snakeA, snakeB).then(function () {
		...
		key: 'updateHead',
		value: function updateHead(snakeA, snakeB) {
			return new Promise(function (resolve, reject) {
		...
		key: 'updateTail',
		value: function updateTail(snakeA, snakeB) {
			return new Promise(function (resolve, reject) {
				// snakeA 与 snakeB 做尾指针比较
				var tailA = snakeA.last(),
				    tailB = void 0;
		...
		value: function wholeUpdate(snakeA, snakeB) {
			...
			try {
				for (var _iterator4 = snakeA[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
					var data = _step4.value.data;
	...
	_createClass(Chain, [{
		...
		value: function at() {
			...
			if (index < this.length / 2) {
				...
				for (var _i = this.length - 1; _i !== index; --_i) {
		...
		value: function setPointer() {
			var addr = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this.POINTER;
		...
		value: function clone() {
			var copy = new Chain();
			var _arr = ["HEAD", "TAIL", "FREE", "length"];
			...
			// FREELIST
			copy.FREELIST = [].concat(_toConsumableArray(this.FREELIST));
			return copy;
		}
		// 删除指定位置的元素
		...
		value: function remove(index) {
			...
			if (index === 0) {
				...
				// 上一个节点
				var prev = this.chain[cur.prev];
				// 下一个节点
				var next = this.chain[cur.next];
				// 回收 FREE
				this.collection();
				// FREE 指向被删除的位置
				this.FREE = cur.index;
				...
				// 链表长度 -1
				var _ref = [next.index, prev.index];
				prev.next = _ref[0];
				next.prev = _ref[1];
				--this.length;
				// 返回 FREE
		...
		key: "insertBefore",
		value: function insertBefore(index, item) {
			// 数组范围之外
			if (index < 0 || index >= this.length) return;
			// 当前节点 
			var cur = this.at(index);
			// 上一个节点
			var prev = this.chain[cur.prev] || { index: -1 };
			// 下一个节点
			var next = cur;
			// 插入新节点
		...
		key: "insertAfter",
		value: function insertAfter(index, item) {
			// 数组范围之外
			if (index < 0 || index >= this.length) return;
			// 当前节点 
			var cur = this.at(index);
			// 下一个节点
			var next = this.chain[cur.next] || { index: -1 };
			// 上一个节点
			var prev = cur;
			// 插入新节点
			cur = this.chain[this.FREE] = {
...
exports.default = function () {
    ...
    var count = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : list.length;
    var filter = arguments[2];

    list = [].concat(list); // 切断与参数list 的耦合
    list = typeof filter === "function" ? list.filter(function () {
        return filter.apply(undefined, arguments);
    }) : list;
    var res = [];
    for (var i = 0; i < count; ++i) {
        var _ref = [list.length, Math.random() * list.length >> 0],
            len = _ref[0],
            randomIndex = _ref[1];
...
ticker.pause = function () {
	return ticker.paused = 1;
}, TweenMax.pauseAll();
ticker.resume = function () {
	return ticker.paused = 0;
}, TweenMax.resumeAll();
ticker.addEventListener = function () {
	var _TweenMax$ticker;

	return (_TweenMax$ticker = TweenMax.ticker).addEventListener.apply(_TweenMax$ticker, arguments);
};
ticker.removeEventListener = function () {
	var _TweenMax$ticker2;

	return (_TweenMax$ticker2 = TweenMax.ticker).removeEventListener.apply(_TweenMax$ticker2, arguments);
};
...
var Timer = function () {
    ...
    _createClass(Timer, [{
        ...
        value: function update() {
            ...
            // 下面是真正的 update
            this.update = function (paused, delta) {
                if (_this2.usingRAF) return;
                // 播放开关 
                _this2.paused == paused || (_this2.paused = paused);
                _this2.tick(delta);
            };
        }
    }]);
...
var SnakeClass = function (_SnakeControl) {
	_inherits(SnakeClass, _SnakeControl);

	function SnakeClass(dom) {
		_classCallCheck(this, SnakeClass);

		return _possibleConstructorReturn(this, (SnakeClass.__proto__ || Object.getPrototypeOf(SnakeClass)).call(this, new _model2.default(), new _view2.default(dom)));
	}
...
},{"./core/control":326,"./core/model":327,"./core/view":328,"babel-polyfill":1}]},{},[337]);
